...
protected:
    ...
    std::unique_ptr<MockGameObjectMediator> mediator_;

    virtual void SetUp() {
        mediator_ = std::unique_ptr<MockGameObjectMediator>(new MockGameObjectMediator());
        std::unique_ptr<MockGameObjectMediator> = new MissileLauncher(Team::FRIENDLY, *mediator_) ;
    }
...
TEST_F(MissileLauncherTest, multiple_missiles) {
    EXPECT_CALL(*mediator_, add_projectile(Team::FRIENDLY, _))
    .Times(3);
...
TEST_F(MissileLauncherTest, correct_params_passed) {
    EXPECT_CALL(*mediator_, add_projectile(Team::FRIENDLY, AllOf(Property(&Missile::x_pos, 11.0),
...
TEST_F(MissileLauncherTest, correct_missiles_passed) {
    ...
    EXPECT_CALL(*mediator_, add_projectile(Team::FRIENDLY, AllOf(Property(&Missile::x_pos, 10.0),
                                           Property(&Missile::y_pos, 10.0),
                                           Property(&Missile::x_vel, 10.0),
                                           Property(&Missile::y_vel, 10.0))));
    EXPECT_CALL(*mediator_, add_projectile(Team::FRIENDLY, AllOf(Property(&Missile::x_pos, 20.0),
...
TEST_F(MissileLauncherTest, check_missiles_correct_team) {
    ...
    EXPECT_CALL(*mediator_, add_projectile(Team::FRIENDLY, _));
    EXPECT_CALL(*mediator_, add_projectile(Team::ENEMY, _));
